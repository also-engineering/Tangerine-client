...
CaseSearchRunView = Backbone.View.extend
  ...
  showOptions: (event) ->
    $target = $(event.target)
    currentLevel = $target.attr('data-level')
  ...
  initialize: (options) ->
    ...
    @caseData.forEach (caseDatum,i) -> caseDatum.push(i)
  ...
  getResult: ->
    caseAttributes = {
      fields   : @fields
      caseData : @caseData[@selected]
    }

    @visibleFields.forEach (field, i) ->
      fieldIndex = @fields.indexOf(field)
      fieldValue = @$el.find("#field-#{fieldIndex}").val()
      caseAttributes.caseData[fieldIndex] = fieldValue
    , @
